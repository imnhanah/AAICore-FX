 ...
 */
export async function getCalendarWeek(storage, offset) {
  const key = `ff-cache:${offset}`;
  let cache = null;
  try {
    const res = await storage.get(key);
    if (res && res.value) cache = JSON.parse(res.value);
  ...
  try {
    ...
    try { await storage.set(key, JSON.stringify({ events, fetchedAt: now })); } catch (e) {}
