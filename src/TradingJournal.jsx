...
function NewTradeModal({ onClose, onSave, editing, typeTags, mistakeTags, onAddTypeTag, ratingStyle }) {
      ...
      <Card className="tj-panel" style={{ marginTop: 14 }}>
        ...
        <div className="tj-scatter-box">
          ...
          {tagStats.map((t) => {
            ...
            const color = t.winRate >= 66 ? "#4DFF66" : t.winRate >= 33 ? "#FBBF24" : "#F87171";
            ...
            return (
              <div key={t.tag} className="tj-scatter-dot-wrap" style={{ left: `${left}%`, top: `${top}%` }}>
                ...
                <div className={`tj-scatter-tooltip ${flipDown ? "tj-scatter-tooltip-below" : ""}`}>
                  ...
                  <div className="tj-scatter-tooltip-row"><span>Win Rate</span><span className="tj-bold">{t.winRate.toFixed(1)}%</span></div>
            ...
            <tbody>
              {[...rows].reverse().map((r) => (
                <tr key={r.key}>
                  ...
                  <td><div className="tj-inline-bar"><div className="tj-bar-track" style={{ width: 90 }}><div className="tj-bar-fill tj-bar-green" style={{ width: `${r.winRate}%` }} /></div><span>{r.winRate.toFixed(0)}%</span></div></td>
    ...
    <>
        ...
        <div className="tj-dow-list">
          {rows.map((r, i) => (
            <div key={r.dow} className="tj-dow-row">
              ...
              <span className="tj-muted-txt tj-dow-wr">{r.winRate.toFixed(0)}% WR</span>
      ...
      <div className="tj-perf-summary-grid" style={{ gridTemplateColumns: "1fr 1fr" }}>
        <Card className="tj-panel tj-perf-summary-card">
          ...
          <div className="tj-muted-txt" style={{ fontSize: 11 }}>{fmtMoney(most.pnl)} · {most.winRate.toFixed(0)}% WR</div>
        </Card>
        <Card className="tj-panel tj-perf-summary-card">
          ...
          <div className="tj-muted-txt" style={{ fontSize: 11 }}>{fmtMoney(least.pnl)} · {least.winRate.toFixed(0)}% WR</div>
        </Card>
      </div>
    </>
  );
...
function NewsPage() {
  ...
  useEffect(() => {
    ...
    getCalendarWeek(storage, weekOffset).then((result) => {
      if (cancelled) return;
      if (!result.events) {
...
function TradingJournalApp({ user, onLogout }) {
  ...
  return (
    <div className={`tj-root ${account.theme === "light" ? "tj-theme-light" : ""}`}>
      ...
      <div className={`tj-sidebar ${sidebarOpen ? "tj-sidebar-shown" : "tj-sidebar-collapsed"}`}>
        <div className="tj-sidebar-scroll">
          ...
          <div className="tj-nav">
            ...
            <button className="tj-nav-item" onClick={() => {
              if (!window.confirm("Delete every trade for this account and reset it to its starting balance? This can't be undone.")) return;
              setAccounts((accs) => accs.map((a) => (a.id === activeId ? { ...a, trades: [], checkins: {} } : a)));
            }}><Trash2 size={16} /> <span>Reset Data</span></button>
            ...
            <div className="tj-account-menu">
              ...
              {accounts.map((a) => (
                <div key={a.id} className={`tj-account-row-wrap ${a.id === activeId ? "tj-account-row-active" : ""}`}>
                  ...
                  {accounts.length > 1 && (
                    <button className="tj-account-delete" title="Delete account" onClick={(e) => {
                      e.stopPropagation();
                      ...
                      setAccounts((accs) => {
                        ...
                        if (a.id === activeId) setActiveId(remaining[0].id);
                        return remaining;
                      });
      ...
      {modal === "newtrade" && <NewTradeModal editing={editingTrade} typeTags={typeTags} mistakeTags={mistakeTags} onAddTypeTag={addTypeTag} ratingStyle={account.ratingStyle} onClose={() => { setModal(null); setEditingTrade(null); }} onSave={saveTrade} />}
...
/* trade log */
.tj-tradelog-controls { display: flex; gap: 8px; flex-wrap: nowrap; align-items: center; overflow-x: auto; padding-bottom: 2px; }
.tj-tradelog-controls .tj-search { flex: 1 1 160px; min-width: 130px; }
.tj-tradelog-controls > *:not(.tj-search) { flex-shrink: 0; }
...
export default function AppRoot() {
  ...
  if (!user) {
    return (<><CustomCursor /><AuthPage storage={storage} onAuthed={(u) => setUser(u)} brand="AAICOREFX" /></>);
  }
